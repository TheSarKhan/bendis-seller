package com.sarkhan.backend.bendisseller.service.impl.product;

import com.sarkhan.backend.bendisseller.dto.product.*;
import com.sarkhan.backend.bendisseller.mapper.comment.CommentMapper;
import com.sarkhan.backend.bendisseller.mapper.product.ProductMapper;
...
import com.sarkhan.backend.bendisseller.model.product.items.ColorAndSize;
...
import com.sarkhan.backend.bendisseller.repository.comment.CommentRepository;
import com.sarkhan.backend.bendisseller.repository.product.ProductRepository;
import com.sarkhan.backend.bendisseller.repository.product.items.PlusRepository;
import com.sarkhan.backend.bendisseller.repository.product.items.ProductUserHistoryRepository;
import com.sarkhan.backend.bendisseller.repository.product.items.UserFavoriteProductRepository;
import com.sarkhan.backend.bendisseller.repository.seller.SellerRepository;
import com.sarkhan.backend.bendisseller.service.CloudinaryService;
import com.sarkhan.backend.bendisseller.service.SellerService;
import com.sarkhan.backend.bendisseller.service.product.ProductService;
import com.sarkhan.backend.bendisseller.service.product.items.CategoryService;
...
import static com.sarkhan.backend.bendisseller.service.impl.product.util.AsyncUtil.*;
import static com.sarkhan.backend.bendisseller.service.impl.product.util.ProductFilterUtil.getByComplexFilteringUseSpecification;
import static com.sarkhan.backend.bendisseller.service.impl.product.util.ProductImageUtil.deleteAllImages;
import static com.sarkhan.backend.bendisseller.service.impl.product.util.ProductImageUtil.uploadImages;
import static com.sarkhan.backend.bendisseller.service.impl.product.util.RecommendationUtil.getRecommendedProduct;
import static com.sarkhan.backend.bendisseller.service.impl.product.util.RecommendationUtil.partialShuffle;
import static com.sarkhan.backend.bendisseller.service.impl.product.util.SellerUtil.*;
...
public class ProductServiceImpl implements ProductService {
    ...
    public ProductServiceImpl(ProductRepository productRepository, ProductUserHistoryRepository historyRepository, CloudinaryService cloudinaryService, CategoryService categoryService, SubCategoryService subCategoryService, SellerService userService, UserFavoriteProductRepository favoriteRepository, @Qualifier("taskExecutor") Executor executor, SellerRepository sellerRepository, PlusRepository plusRepository, CommentRepository commentRepository) {
    ...
    @Override
    public Product add(ProductRequest request, List<MultipartFile> images) throws IOException, AuthException {
        Seller user = getCurrentSeller(userService, log);
        log.info("{} try to create product", user.getFullName());
    ...
    public ProductResponseForHomePage getForHomePage() {
        ...
        return new ProductResponseForHomePage(shuffleAndDecreaseSize(productRepository.getFamousProducts(pageable)), shuffleAndDecreaseSize(productRepository.getDiscountedProducts(pageable)), shuffleAndDecreaseSize(productRepository.getMostFavoriteProducts(pageable)), shuffleAndDecreaseSize(productRepository.getFlushProducts(pageable)), recommendedProduct());
    }
    ...
    public CompletableFuture<ProductResponseForSearchByName> searchByName(String name) {
        ...
        var combinedSubCategoriesFuture = combineSubCategories(subCategoriesFuture, searchedSubCategoriesFuture);

        var subCategoryProductsFuture = getProductsBySubCategories(combinedSubCategoriesFuture, productRepository, executor);

        SecurityContext context = SecurityContextHolder.getContext();

        return CompletableFuture.allOf(productsFuture, subCategoryProductsFuture).thenApply(unused -> {
            ...
            try {
                ...
                return new ProductResponseForSearchByName(name, mapProductsToProductsResponse(products.stream().toList()));
            } catch (Exception e) {
    ...
    public ProductResponseForSelectedSubCategoryAndComplexFilter getBySubCategoryId(Long subCategoryId) {
        ...
        List<ColorAndSize> colorAndSizes = productsBySubCategory.stream().map(Product::getColorAndSizes).reduce((a, b) -> {
            a.addAll(b);
            return a;
        }).orElse(new ArrayList<>());

        return new ProductResponseForSelectedSubCategoryAndComplexFilter(mapProductsToProductsResponse(productsBySubCategory), specifications, colorAndSizes.stream().map(ColorAndSize::getColor).collect(Collectors.toSet()).stream().toList(), colorAndSizes.stream().map(colorAndSize -> colorAndSize.getSizeStockMap().keySet()).reduce((a, b) -> {
            a.addAll(b);
            return a;
        }).orElse(new HashSet<>()).stream().toList(), new ProductFilterRequest(subCategoryId, null, null, null, null, null, null, null));
    }
    ...
    public CompletableFuture<ProductResponseForSelectedSubCategoryAndComplexFilter> getByComplexFiltering(ProductFilterRequest request) {
        ...
        CompletableFuture<List<Product>> productsFuture = CompletableFuture.supplyAsync(() -> getByComplexFilteringUseSpecification(request, productRepository));

        CompletableFuture<List<Product>> productsBySubCategoryFuture = CompletableFuture.supplyAsync(() -> productRepository.getBySubCategoryId(request.subCategoryId()), executor);

        CompletableFuture<List<String>> specsFuture = CompletableFuture.supplyAsync(() -> subCategoryService.getById(request.subCategoryId()).getSpecifications(), executor);

        SecurityContext context = SecurityContextHolder.getContext();

        return CompletableFuture.allOf(productsFuture, specsFuture, productsBySubCategoryFuture).thenApply(unused -> {
            SecurityContextHolder.setContext(context);
            try {
                List<ColorAndSize> colorAndSizes = productsBySubCategoryFuture.get().stream().map(Product::getColorAndSizes).reduce((a, b) -> {
                    a.addAll(b);
                    return a;
                }).orElse(new ArrayList<>());
                return new ProductResponseForSelectedSubCategoryAndComplexFilter(mapProductsToProductsResponse(productsFuture.get()), specsFuture.get(), colorAndSizes.stream().map(ColorAndSize::getColor).collect(Collectors.toSet()).stream().toList(), colorAndSizes.stream().map(colorAndSize -> colorAndSize.getSizeStockMap().keySet()).reduce((a, b) -> {
                    a.addAll(b);
                    return a;
                }).orElse(new HashSet<>()).stream().toList(), request);
            } catch (Exception e) {
    ...
    public List<Product> getAllFavorite() throws AuthException {
        ...
        return productRepository.getAllFavorite(getCurrentSeller(userService, log).getId());
    }
    ...
    public void giveRating(Long id, Double rating) throws AuthException {
        ...
        log.info("{} try to give rating. Product name : {}", user.getFullName(), product.getName());

        Map<Long, Double> ratings = product.getRatings();
        if (ratings.containsKey(user.getId())) {
            log.warn("{} try to give additional rating.", user.getFullName());
    ...
    public ProductResponseForGetSingleOne toggleFavorite(Long id) throws AuthException {
        ...
        log.info("{} pres favorite button. Product name : {}", user.getFullName(), product.getName());

        Long userId = user.getId();
        Optional<UserFavoriteProduct> favorite = favoriteRepository.getByProductIdAndUserId(product.getId(), userId);
        ...
        if (favorite.isPresent()) {
            ...
            product.setFavoriteCount(product.getFavoriteCount() + 1);
            favoriteRepository.save(UserFavoriteProduct.builder().userId(userId).productId(product.getId()).build());
        }
        productRepository.save(product);
        return mapProductToResponse(product);
    }
    ...
    @Override
    public ProductResponseForGetSingleOne update(Long id, ProductRequest request, List<MultipartFile> newImages) throws IOException, AuthException {
        ...
        log.info("{} try to update product. Id : {}", user.getFullName(), id);
    ...
    public void delete(Long id) throws AuthException {
        ...
        log.warn("{} delete product. Id : {}", user.getFullName(), id);
        if (Role.ADMIN.equals(user.getRole()) || getById(id).getSellerId().equals(user.getId())) {
            productRepository.deleteById(id);
        } else {
            log.warn("{} cannot delete this product.", user.getFullName());
        }
    }
    ...
    private List<Product> shuffleAndDecreaseSize(List<Product> products) {
        return partialShuffle(products, shuffleProbability, recommendedProductMaxSize).stream().limit(homePageCount).toList();
    }

    private List<Product> recommendedProduct() {
        return getRecommendedProduct(historyRepository, productRepository, subCategoryService, userService, log, recommendedProductMaxSize, shuffleProbability, maxSwapDistance);
    }
    ...
    private ProductResponseForGetSingleOne mapProductToResponse(Product product) {
        ...
        try {
            Seller currentUser = getCurrentSeller(userService, log);
        ...
        return ProductMapper.productToProductResponseForGetSingleOne(product, subCategory, seller, pluses, CommentMapper.mapCommentsToCommentResponses(byProductId), isFavorite);
    }

    private List<ProductResponseForGroupOfProduct> mapProductsToProductsResponse(List<Product> products) {
        try {
            Seller currentUser = getCurrentSeller(userService, log);
            Map<Product, Boolean> productResponseMap = mapProductsToResponseWhenUserLogin(products, currentUser);
    ...
    private Map<Product, Boolean> mapProductsToResponseWhenUserLogin(List<Product> products, Seller currentUser) {
        HashMap<Product, Boolean> productIsFavoriteMap = new HashMap<>();
        for (Product product : products)
            productIsFavoriteMap.put(product, favoriteRepository.getByProductIdAndUserId(product.getId(), currentUser.getId()).isPresent());
