...
public class SellerUtil {
    ...
    public static Seller getCurrentSeller(SellerService userService, Logger log) throws AuthException {
        SellerUserDetails details = (SellerUserDetails) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        if (details.equals("anonymousUser")) {
            log.warn("User doesn't login!!!");
            throw new AuthException("User doesn't login!!!");
        }
        return userService.getByBrandEmail(details.getUsername());
    }
